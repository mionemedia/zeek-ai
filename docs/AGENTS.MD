...
## Phi-3 Mini Attribution (Mini In‑App AI)

We use Microsoft Phi‑3 Mini as the default small model for the Agents System Prompt Creator and future light UI functions.

- License: MIT (broad usage rights; attribution required; provided “as is”).
- Source: Microsoft (Hugging Face Hub, GitHub), also available via Ollama.

How to obtain locally (recommended via Ollama):
...
- The model runs locally; no data leaves the device.
- Default quantization recommended: Q4 (~2 GB disk).
- You may switch models (e.g., Gemma 2B, Qwen 2B, Yi 2B) in Settings when added.